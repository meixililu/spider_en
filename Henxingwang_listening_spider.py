...
def nowplaying_movies(url,publish_time,img_url):
    ...
    mp3div = soup.find('div',class_='mp3player')
    if mp3div != None:
        mp3_a = mp3div.find('a',attrs={"target": "_blank"})
        if mp3_a != None:
            media_url = mp3_a['href']
    ...
    if is_exit(title):
        ...
        mComposition.set('category', 'listening')
        mComposition.set('media_url', media_url)
        mComposition.set('type', 'mp3')
        mComposition.save()
        # print('save item')
...
def is_exit(str):
    ...
    query.equal_to('category', 'listening')
    querys = query.find()
    return len(querys) > 0
...
def task_Henxingwang_listening_spider():
    global item_id
    ...
    # for i in range(1,2):
        if i == 1:
            url = 'http://www.hxen.com/englishlistening/index.html'
        else:
            url = 'http://www.hxen.com/englishlistening/index_%d.html'% (i)
        # print('root url:'+url)
        get_all_link(url)
