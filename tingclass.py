...
def parse_detail(url):
    ...
    media_url = ''
    lrc_url = ''
    publish_time = datetime.strptime('2018-08-01 08:00:00', "%Y-%m-%d %H:%M:%S")
    headers = {
        'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/47.0.2526.80 Safari/537.36'}
    req = requests.get(url, headers=headers)
    req.encoding = 'utf-8'
    if 'video.qq' in req.text or 'v.qq.com' in req.text or 'player.youku.com' in req.text \
            or 'player.video.qiyi.com' in req.text or 'player.pps.tv' in req.text:
        return False
...
def parse_category_list(url):
    ...
    try:
        ...
        for link in alinks:
            ...
            print nextLink
            print category_2
            if not parse_page_num(nextLink):
...
def tingclass_spoken_english():
    ...
    type_name = u'英语口语'
    urls = ["http://www.tingclass.net/list-45-1.html"]
    for url in urls:
